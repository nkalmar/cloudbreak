...
public class DistroXEncryptedVolumeTest extends AbstractE2ETest {
    ...
    protected void setupTest(TestContext testContext) {
        notSupportedCloudPlatform(CloudPlatform.GCP);
    ...
    @Test(dataProvider = TEST_CONTEXT, description = "KNOWN ISSUE: CB-15601 Azure Disk Encryption Set creation throws" +
            " PSQLException if environment name too long")
    @UseSpotInstances
    @Description(
    ...
    private EnvironmentTestDto verifyEnvironmentResponseDiskEncryptionKey(TestContext testContext, EnvironmentTestDto testDto,
        ...
        if (CloudPlatform.AWS.name().equals(environment.getCloudPlatform())) {
            ...
            environmentName = testDto.getRequest().getName();
            verifyDiskEncryptionKey(testDto.getCloudPlatform(), encryptionKeyArn);
            ...
            environmentName = testDto.getRequest().getName();
            verifyDiskEncryptionKey(testDto.getCloudPlatform(), diskEncryptionSetId);
        } else {
    ...
    private void verifyDiskEncryptionKey(CloudPlatform cloudPlatform, String encryptionKey) {
    ...
    private void verifyVolumeEncryptionKey(CloudPlatform cloudPlatform, String resourceName, List<String> instanceIds,
            CloudFunctionality cloudFunctionality) {
        if (CloudPlatform.AWS.equals(cloudPlatform)) {
