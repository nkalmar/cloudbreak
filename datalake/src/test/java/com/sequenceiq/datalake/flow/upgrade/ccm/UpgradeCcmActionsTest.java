...
class UpgradeCcmActionsTest {
    ...
    void finishedHappyPath() {
        ...
        verify(sdxStatusService).setStatusForDatalakeAndNotify(DatalakeStatusEnum.RUNNING, "Datalake is running", SDX_ID);
    ...
    private void testErrorInSdxStatusService(Supplier<Action<?, ?>> upgradeAction) {
        when(sdxStatusService.setStatusForDatalakeAndNotify(any(), any(), anyLong())).thenThrow(new IllegalStateException("fuck you"));
